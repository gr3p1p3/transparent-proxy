...
module.exports = function parseDataToObject(data, isResponse = false, chunked = false) {
    ...
    if (!chunked) {
        let [headers, body] = [dataString.slice(0, splitAt), dataString.slice(splitAt + 1)];
        ...
        const startWithLFCRL = body.indexOf(LF + CRLF) === 0;
        if (startWithLFCRL) {
            body = body.slice((LF + CRLF).length, body.length);
        }
        if (body) {
